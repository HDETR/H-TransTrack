...
def train_hybrid(outputs, targets, k_one2many, criterion, lambda_one2many):
    ...
    queries_per_group = (
        outputs["pred_logits_one2many"].shape[1] // k_one2many
    )
    for gt_idx in range(k_one2many):
        queries_idx_start = gt_idx * queries_per_group
        queries_idx_end = (gt_idx + 1) * queries_per_group
        outputs_one2many = dict()
        outputs_one2many["pred_logits"] = outputs["pred_logits_one2many"][
            :, queries_idx_start: queries_idx_end, :
        ]
        outputs_one2many["pred_boxes"] = outputs["pred_boxes_one2many"][
            :, queries_idx_start: queries_idx_end, :
        ]
        outputs_one2many["aux_outputs"] = list()
        for layer_idx, layer_aux_outputs in enumerate(
            outputs["aux_outputs_one2many"]
        ):
            output_one2many_dict = dict()
            output_one2many_dict["pred_logits"] = layer_aux_outputs[
                "pred_logits"
            ][:, queries_idx_start: queries_idx_end, :]
            output_one2many_dict["pred_boxes"] = layer_aux_outputs[
                "pred_boxes"
            ][:, queries_idx_start: queries_idx_end, :]
            outputs_one2many["aux_outputs"].append(output_one2many_dict)

        loss_dict_multi = criterion(outputs_one2many, targets)
        for key, value in loss_dict_multi.items():
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    epoch: int, max_norm: float = 0, fp16=False, k_one2many=None, lambda_one2many=1.0):
    model.train()
    criterion.train()
...
#     metric_logger.add_meter('class_error', utils.SmoothedValue(window_size=1, fmt='{value:.2f}'))
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if tracker is not None and not det_val:
            if phase != 'train':
